...
{
	make_ortho(m_ortho, 0.0f, width, height, 0.0f, -1.0f, 1.0f);
}
...
void gles2_renderer::use_quad_program()
{
	...
	if (m_last_program != m_quad_program)
	{
		...
		glUniformMatrix4fv(m_uniform_ortho_quad, 1, GL_FALSE, m_ortho);
...
void gles2_renderer::use_line_program()
{
	...
	if (m_last_program != m_line_program)
	{
		...
		glUniformMatrix4fv(m_uniform_ortho_line, 1, GL_FALSE, m_ortho);
